...
X_transformed = np.array(df["Altura(mts)"]).reshape(-1, 1)
Y_transformed = np.array(df["Peso(kg)"]).reshape(-1, 1)

# Visualización de los datos
sb.scatterplot(
    x="Altura(mts)", y="Peso(kg)", data=df, hue="Peso(kg)", palette="coolwarm"
)
plt.show()
...
print(
    f"La predicción es de: {prediction.item():.2f} kg para una persona de {altura} mts de altura"
)
...
print(
    f"La predicción es de: {prediction_2.item():.2f} kg para una persona de {height} mts de altura"
)
